...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/Moviezs_Store")]])
    )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_sticker("CAACAgEAAxkBAAIjomVM82FJOWOsocbClnCVRytUvD4sAAITAgACcW0hRC6-AAHEGmIBkx4E")
            search = search.lower()
            find = search.split(" ")
            search = ""
